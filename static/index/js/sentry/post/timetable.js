...
function getServices() {
    ...
    $.ajax({ url:'/post/timetable/get_services/', type:'get', dataType:'json', data:ajax_array,
...
function refreshTimetable() {
    ...
    $.ajax({ url:'/post/timetable/get_timetable/', type:'get', dataType:'json', data:ajax_array,
        success: function(data){
            shifts = data['shifts'];
...
function setTable(data) {
    for(var service_id in data['post']) {
        ...
        for(var user_id in service['users']) {
            ...
            tr.find('td:eq(0) a').attr('href', '/post/personal/?user_id='+user['user_id']);
