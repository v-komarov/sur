$(document).ready(function(){
    ...
    $('#contract, #object_list').on('click', 'tr', function(){
        ...
        if(action=='contract'){
            ...
            object_Edit(
                ...
                $(this).parents('.object__item').attr('bind_id')
            );
        }
        else if(action=='cost'){
            cost_Edit( $(this).parents('[bind_id]').attr('bind_id'), $(this).attr('object_cost_id') );
        }
        else if(action=='event'){
            var contract_id = $(this).parents('#contract').attr('contract_id');
            var bind_id = $(this).parents('[bind_id]').attr('bind_id');
            var object_id = $(this).parents('[object_id]').attr('object_id');
            event_Edit(contract_id, bind_id, object_id, $(this).attr('event_type'), $(this).attr('event_id'));
        }
            ...
            if(client_object_connect=='') popMessage('Не произведено подключение','red');
                ...
                cost_Edit(bind_id, object_cost_id, 'pause');
            }
        }
    ...
    $(document).on('click', 'td[name=pause_list] div.pause__item', function(){
        cost_Edit( $(this).parents('.object__item').attr('bind_id'), $(this).attr('object_cost_id') );
    });

    $(document).on('click', '.btn_ui:not(.disabled)', function(){
        var action = $(this).attr('action');
        ...
        if(action=='contract_reset') contract_Reset();
        ...
        else if(action=='object_hide') object_Show('hide', $(this).parents('.object__item').attr('bind_id'));
        else if(action=='object_show') object_Show('show', $(this).parents('.object__item').attr('bind_id'));
        else if(action=='object_edit') object_Edit(
            $(this).parents('.object__item').attr('object_id'),
            $(this).parents('.object__item').attr('bind_id')
        );
            ...
            object_Archive($(this).parents('[bind_id]').attr('bind_id'));
        }
        else if(action=='object_unarchive') {
            object_Archive($(this).parents('[bind_id]').attr('bind_id'), 'unarchive');
        }

        else if(action=='search_archive') {
            searchShow();
        }
